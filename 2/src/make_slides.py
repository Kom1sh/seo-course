...
def graph(orphan=True, w=640, h=420):
    ...
    for k, (x, y, lab) in nodes.items():
        ...
        nw = max(104, len(lab) * 8.4 + 18)
        out.append(f'<rect x="{x-nw/2:.1f}" y="{y-26}" width="{nw:.1f}" height="52" rx="12" fill="{fill}" stroke="{stroke}" stroke-width="2"/>')
    ...
    ox, oy, _ = nodes["o"]
    out.append(f'<text x="{ox-52}" y="{oy+52}" text-anchor="start" style="font-size:16px;font-weight:500;fill:#600000">ни одной входящей ссылки</text>')
    out.append("</svg>"); return "".join(out)
...
def shot_marks(url, img, marks, w=None):
    """Скриншот в окне браузера с номерами поверх. marks: (номер, x 0..1, y 0..1 по картинке)."""
    mk = "".join(f'<span class="mk" style="left:{x*100:.1f}%;top:calc(40px + (100% - 40px) * {y:.3f})">{n}</span>' for n, x, y in marks)
    return f'<div class="shotmk">{browser(url, img, w)}{mk}</div>'

def ann(items):
    out = []
    for n, h, hu, se in items:
        out.append(f'<div class="it"><span class="m">{n}</span><div><h5>{h}</h5>'
                   f'<p><em class="hu">Человеку:</em> {hu}<br><em class="se">Поисковику:</em> {se}</p></div></div>')
    return '<div class="ann">' + "".join(out) + '</div>'
...
S.append(slide(top("Школа Икс, факультатив. Встреча 2 из 5") +
  ...
  '<p class="sub">Заходим в Вордстат, собираем кластеры, делаем под них страницы и статьи, связываем ссылками и проверяем. На живых данных Мапки и techkio.</p>'
...
# 03 план
S.append(slide(top("План на сегодня") + '<h1>Шесть шагов за полтора часа: от Вордстата до проверки готового сайта</h1>'
  '<div class="body"><div class="pipe six" style="margin:auto 0">'
  + step("1", "Вордстат", "15 минут. Что ищут, как считать частоту, чем ещё собирают.", "", "search", 0) + arrow()
  + step("2", "Кластеры", "12 минут. Чистка и правило «одно намерение — одна страница».", "", "layers", 1) + arrow()
  + step("3", "Страницы", "15 минут. Дерево сайта и разбор посадочной.", "", "sitemap", 2) + arrow()
  + step("4", "Блог", "15 минут. Зачем он, какие бывают статьи, как писать.", "", "doc", 3) + arrow()
  + step("5", "Ссылки и проверка", "10 минут. Перелинковка, сироты, SiteAnalyzer.", "", "link", 4) + arrow()
  + step("6", "Практикум", "20 минут. Ядро и дерево для ниши, которую выберем.", "", "pen", 5)
...
S.append(slide(top("Операторы Вордстата") + '<h1>Шесть операторов превращают Вордстат из счётчика в инструмент</h1>'
  ...
  + '</div><p class="foot">Операторы интерфейса 2026 года: wehaveidea.ru, pr-cy.ru. Минус-слова с августа 2026 вписывают в отдельное поле.</p></div>', "Операторы"))
...
S.append(slide(top("Чистка") + '<h1>Половину собранного выкидывают или откладывают до кластеризации</h1>'
  ...
  for k, h, c, q, p in BUCKETS) + '</div><p class="foot">Все запросы — подсказки Яндекса по нашей нише, 14 сентября 2026. Слова из красной колонки сразу пишем в поле минус-слов Вордстата.</p></div>', "Чистка"))
...
S.append(divider("04", "Структура сайта", "Как разложить кластеры в дерево, которое понятно и человеку, и роботу.",
                 ["Дерево кластеров и типы страниц", "Мапка: дерево из спроса и фасеты", "Посадочная для человека и для робота", "Адреса, глубина, перелинковка", "Страницы-сироты", "Проверка расширением SiteAnalyzer"]))
...
S.append(slide(top("Фасеты") + '<h1>Направление и район — отдельная страница, если есть спрос и секции</h1>'
  ...
  '<div style="flex:1;display:flex;flex-direction:column;gap:14px">'
  + sticky("yel", "Порог: от трёх секций", f'На Северном {MPK["angliyskiy_severny"]} секций английского из {MPK["angliyskiy_all"]} — хватает на отдельную страницу. Где секций меньше трёх, страница закрыта от индексации: пустая посадочная тянет вниз весь сайт.')
  + sticky("teal", "Спрос на районы есть", f'Каждый шестой запрос к Мапке содержит район: {fmt(D["districts"]["queries"])} из {fmt(D["districts"]["total"])} за месяц. Больше всего — Северный, Левенцовка и Западный.')
...
# посадочная: первый экран
S.append(slide(top("Посадочная, первый экран") + '<h1>Каждый блок посадочной работает дважды: на человека и на поисковик</h1>'
  '<div class="body"><div class="row fill" style="gap:28px;align-items:center">'
  f'<div style="flex:1.12">{shot_marks("мапка.рф/sektsii/angliyskiy-na-severnom", "mapka_combo.webp", [(1, .372, .285), (2, .372, .445), (3, .372, .575), (4, .405, .795), (5, .80, .27)])}</div>'
  '<div style="flex:1">' + ann([
      (1, "Заголовок с запросом и районом", "видно, что попал по адресу.", "заголовок H1 совпадает с запросом."),
      (2, "Фильтры и поиск", "сузить выбор под себя.", "не должны плодить сотни пустых адресов."),
      (3, "Карточки: цена, рейтинг, адрес", "сравнить школы, не уходя со страницы.", "17 названий текстом и ссылки на карточки."),
      (4, "Кнопка «Подробнее»", "следующий шаг к звонку.", "ссылка вниз по дереву, к карточке школы."),
      (5, "Карта", "видно, что ближе к дому.", "карту не видит, поэтому рядом список.")]) + '</div>'
  '</div><p class="foot">мапка.рф/sektsii/angliyskiy-na-severnom, снимок 14 сентября 2026. В разметке страницы — список организаций, хлебные крошки и частые вопросы.</p></div>', "Посадочная: первый экран"))

# посадочная: блок под картой
S.append(slide(top("Посадочная, под картой") + '<h1>Под картой — текст, частые вопросы и ссылки на соседние районы</h1>'
  '<div class="body"><div class="row fill" style="gap:28px;align-items:center">'
  f'<div style="flex:1.12">{shot_marks("мапка.рф/sektsii/angliyskiy-na-severnom", "mapka_landing_seo.webp", [(1, .09, .20), (2, .09, .58), (3, .09, .88)])}</div>'
  '<div style="flex:1">' + ann([
      (1, "Заголовок и два абзаца о направлении", "как выбирать и что сравнивать.", "свой текст; без него страница — просто список, как у всех каталогов."),
      (2, "Частые вопросы", "сколько секций, как выбрать, с какого возраста, сколько стоит.", "подзапросы кластера, размечены как FAQ."),
      (3, "«Смотрите также»", "футбол, плавание, гимнастика в том же районе.", "соседние посадочные получают ссылку — сирот нет.")])
  + '<p style="margin-top:20px;font:400 19px/1.45 var(--ft);color:var(--charcoal)">Маркетинг решает, захочет ли человек записаться. SEO решает, увидит ли он страницу вообще. На посадочной нужно и то и другое, и оба живут в одних и тех же блоках.</p></div>'
  '</div><p class="foot">Тот же адрес, блок ниже карты. 1 741 слово на странице, 45 внутренних ссылок.</p></div>', "Посадочная: под картой"))

# красиво и SEO-корректно
BEAUTY = [("Текст на картинке баннера", "картинку без заголовка", "текст вёрсткой поверх фона, у картинки подпись alt"),
          ("Цены и отзывы во вкладках, которые грузятся по клику", "пустые вкладки: содержимое приходит скриптом", "всё сразу в HTML, вкладка только прячет и показывает"),
          ("Кнопка «Подробнее» на каждой карточке", "сто одинаковых ссылок, ни одна ничего не говорит", "ссылкой служит название школы, кнопка его дублирует"),
          ("Три крупных заголовка ради ритма", "три H1 — непонятно, какой главный", "один H1 с запросом, остальные H2 и H3"),
          ("Видео на весь первый экран", "тяжёлую страницу, которая долго открывается с телефона", "лёгкая обложка, видео запускается по нажатию"),
          ("Карта вместо списка", "ни одной организации: карта рисуется скриптом", "карта рядом со списком, как у Мапки")]
S.append(slide(top("Красиво и SEO-корректно") + '<h1>Где красивая посадочная ломает SEO и как сделать и то и другое</h1>'
  '<div class="body"><div class="tbl dense fill" style="grid-template-columns:minmax(0,1fr) minmax(0,1fr) minmax(0,1.1fr)">'
  '<div class="h">Хотят красиво</div><div class="h">Что видит поисковик</div><div class="h geo">Как совместить</div>'
  + "".join(f'<div class="k" style="font-size:20px">{a}</div><div>{b}</div><div class="geo">{c}</div>' for a, b, c in BEAUTY)
  + '</div><p class="foot">Правило одно: всё, что важно человеку, должно лежать в HTML текстом. Тогда это увидит и поисковик, и ИИ-ассистент.</p></div>', "Красиво и SEO-корректно"))
...
# сироты
S.append(slide(top("Страницы-сироты") + '<h1>Гайд по Steam Deck получил две входящие ссылки в день публикации</h1>'
  '<div class="body"><div class="row" style="gap:26px;align-items:center;margin-bottom:18px">'
  f'<div class="orph" style="flex:none;width:470px"><div style="font:500 18px/1.3 var(--ft);color:var(--coral-d);margin-bottom:6px">Без ссылок страница — сирота</div>{graph()}</div>'
  f'<div style="flex:1;min-width:0"><div style="font:500 18px/1.3 var(--ft);color:var(--moss);margin-bottom:6px">Так сделано: ссылка в тексте на techkio.ru/download</div>'
  f'{browser("techkio.ru/download", "techkio_download_link.webp", 660)}</div>'
  '</div><div class="g3 cols" style="margin-bottom:auto">'
  '<div><h5>Откуда берутся сироты</h5><p>Новую страницу забыли прилинковать. Страница открывается только через поиск по сайту. Ссылка сделана кнопкой на скрипте, а не обычной ссылкой.</p></div>'
  '<div><h5>Как найти</h5><p>Сверить адреса из sitemap.xml с тем, что нашёл обход по ссылкам. На хабах посмотреть расширением SiteAnalyzer, куда ведут внутренние ссылки.</p></div>'
  '<div><h5>Как починить</h5><p>У каждой страницы минимум два входа: из меню или хаба и с соседней страницы или статьи. Ставить их в день публикации.</p></div>'
  '</div></div>', "Страницы-сироты"))

# расширение SiteAnalyzer
CHECK = [("1", "Один H1 и его текст"), ("2", "Title и description, их длина"), ("3", "H2 и H3 идут по порядку"),
         ("4", "Внутренние ссылки: куда ведут"), ("5", "noindex и canonical"), ("6", "Подписи alt у картинок")]
S.append(slide(top("Проверка структуры") + '<h1 class="md">SiteAnalyzer за минуту показывает заголовки, ссылки и мету любой страницы</h1>'
  '<div class="body"><div class="row fill" style="gap:28px;align-items:center">'
  '<div style="flex:none;width:340px;display:flex;flex-direction:column;gap:18px">'
  '<div class="qrbox"><img src="assets/qr-siteanalyzer.svg" alt="QR-код: SiteAnalyzer SEO Tools в магазине Chrome"><p>Наведите камеру телефона — откроется расширение в магазине Chrome</p></div>'
  '<div style="font:400 17px/1.4 var(--ft);color:var(--charcoal)">Работает в Chrome и Яндекс Браузере. 10 000 пользователей, оценка 4,9.</div></div>'
  '<div style="flex:1;min-width:0;display:flex;flex-direction:column;gap:16px">'
  f'{browser("chromewebstore.google.com · SiteAnalyzer SEO Tools", "cws_siteanalyzer.webp", 680)}'
  '<div class="lst3">' + "".join(f'<div><b>{n}</b>{t}</div>' for n, t in CHECK) + '</div></div>'
  '</div><p class="foot">Страница расширения в магазине Chrome, 24 сентября 2026. Разработчик — site-analyzer.pro, регистрация не нужна.</p></div>', "Расширение SiteAnalyzer"))

S.append(divider("05", "Блог", "Зачем он сайту, который продаёт, какие бывают статьи и как их писать под поиск и под ИИ.",
                 ["Зачем блог коммерческому сайту", "Шесть типов статей", "Как пишется статья", "Разбор настоящего гайда", "Статья ведёт на посадочные", "Статья под ИИ-ответ"]))

# зачем блог
WHY = [("Собирает вопросы, которые не закрыть посадочной", "«как выбрать», «сколько стоит», «куда отдать в 5 лет» — на посадочной их не разобрать, не испортив её."),
       ("Ведёт робота и читателя к посадочным", f'эта статья ссылается на /sektsii/besplatnye, у которой {fmt(D["besplatno"]["entries"])} входов из поиска за месяц.'),
       ("Показывает, что сайт делают люди", "автор «Редакция Мапки.рф», дата «обновлено 1 сентября 2026», своё фото."),
       ("Даёт ИИ-ассистентам, что цитировать", "ответы в начале, списки, цифры и частые вопросы.")]
S.append(slide(top("Зачем блог") + '<h1>Статья про бесплатные секции — первая страница Мапки по кликам из Google</h1>'
  '<div class="body"><div class="row fill" style="gap:28px;align-items:center">'
  f'<div style="flex:1.1">{browser("мапка.рф/blog/besplatnye-sekcii-dlya-detey-rostov", "mapka_article_top.webp")}</div>'
  f'<div style="flex:1">{plist(WHY, "check")}</div>'
  '</div><p class="foot">Google Search Console: 14 кликов и 251 показ за 13 августа — 12 сентября 2026, больше, чем у любой другой страницы Мапки. Входы на посадочную — Яндекс Метрика, тот же период.</p></div>', "Зачем блог"))

# шесть типов статей
TYPES = [("Инструкция", "как сделать механический пресс в create", "шаги по порядку, схема, ошибки, вопросы", "techkio /guide/create-press"),
         ("Подборка мест", "бесплатные секции для детей в ростове на дону", "места по группам, как записаться, вопросы", "Мапка /blog/besplatnye-sekcii-dlya-detey-rostov"),
         ("Как выбрать", "как выбрать спортивную секцию для ребенка", "критерии, чек-лист, частые ошибки", "Мапка /blog/kak-vybrat-sekciyu-dlya-rebenka"),
         ("Сколько стоит", "144 запроса к Мапке за месяц со словом «цены»", "таблица цен из своих данных", "Мапка /blog/skolko-stoyat-detskie-sekcii-rostov"),
         ("Порядок", "катаклизм порядок боссов", "этапы по очереди, что нужно на каждом", "techkio /guide/cataclysm-poryadok-bossov, 305 входов"),
         ("Идеи", "что можно построить в create", "список идей с картинками", "techkio /blog/chto-postroit-v-create, 195 входов")]
S.append(slide(top("Типы статей") + '<h1>Тип статьи выбирают по запросу: у каждого своя структура</h1>'
  '<div class="body"><div class="tbl dense fill" style="grid-template-columns:190px minmax(0,1.05fr) minmax(0,.9fr) minmax(0,1fr)">'
  '<div class="h">Тип</div><div class="h">Настоящий запрос</div><div class="h">Как устроена</div><div class="h">Пример</div>'
  + "".join(f'<div class="k" style="font-size:20px">{a}</div><div class="mono" style="font-size:17px;color:var(--ink)">{b}</div><div>{c}</div><div class="mono" style="font-size:16px;color:var(--moss)">{d}</div>' for a, b, c, d in TYPES)
  + f'</div><p class="foot">Запросы — подсказки Яндекса, 24 сентября 2026, и Яндекс Вебмастер. Входы — Яндекс Метрика, {PER}.</p></div>', "Типы статей"))

# как пишется статья
S.append(slide(top("Как пишется статья") + '<h1>Из шести шагов написания статьи текст — только четвёртый</h1>'
  '<div class="body"><div class="pipe six" style="margin:auto 0">'
  + step("1", "Запрос", "Берём из ядра кластер вопросов. Одна статья — один кластер.", "grey", "target", 0) + arrow()
  + step("2", "Топ-10", "Смотрим, какие блоки есть у всех и чего не хватает никому.", "grey", "search", 1) + arrow()
  + step("3", "План", "Подзаголовки H2 — это подзапросы: подсказки, вопросы из Вебмастера.", "grey", "list", 2) + arrow()
  + step("4", "Текст", "Ответ в первом абзаце, дальше детали. Свои цифры, фото, таблицы.", "yel", "pen", 3) + arrow()
  + step("5", "Ссылки", "В тексте — на 1–3 посадочные, внизу «читайте также».", "grey", "link", 4) + arrow()
  + step("6", "Уход", "Автор и дата на странице, обновление к сезону.", "grey", "refresh", 5)
  + '</div><p class="foot">Шаги 1–3 занимают больше времени, чем сам текст. Без них выходит статья, которую никто не ищет.</p></div>', "Как пишется статья"))

# разбор гайда
OUTL = [("l2", "H1", "Механический пресс в Create: крафт, установка и что он делает", "запрос и обещание"),
        ("l3", "", "Первый абзац: из чего собирается пресс и зачем он нужен", "ответ в начале"),
        ("l2", "H2", "Почему пресс ставят раньше, чем начинают строить", "зачем это читать"),
        ("l2", "H2", "Схемы крафта", "картинки со схемами"),
        ("l2", "H2", "Порядок действий — шесть шагов", "инструкция"),
        ("l2", "H2", "Что во что прессуется", "таблица"),
        ("l2", "H2", "На чём застревают — пять ошибок", "частые проблемы"),
        ("l2", "H2", "Как это работает на K.I.O", "свой опыт"),
        ("l2", "H2", "Частые вопросы — шесть, дословно как запросы", "вопросы под поиск и ИИ"),
        ("l2", "H2", "Готов встать за пульт?", "призыв к действию")]
S.append(slide(top("Разбор гайда") + '<h1>Гайд techkio про пресс — восемь разных видов текста под одним заголовком</h1>'
  '<div class="body"><div class="row fill" style="gap:28px;align-items:center">'
  '<div class="outl" style="flex:1.25">' + "".join(f'<div class="o {lv}"><span><b>{h}</b>{t}</span><em>{w}</em></div>' for lv, h, t, w in OUTL) + '</div>'
  f'<div style="flex:1">{browser("techkio.ru/guide/create-press", "techkio_press.webp")}'
  '<p style="font:400 18px/1.4 var(--ft);color:var(--charcoal);margin-top:12px">1 490 слов. Разметка HowTo для шагов и FAQ для вопросов. На эту страницу Google сам собрал 27 разных запросов.</p></div>'
  '</div><p class="foot">Структура снята с живой страницы techkio.ru/guide/create-press 24 сентября 2026.</p></div>', "Разбор гайда"))

# статья ведёт на посадочные
LNK = [("«бесплатных секций» в тексте", "ведёт на /sektsii/besplatnye — посадочную под тот же спрос"),
       ("«рисование», «музыка» в тексте", "ведут на посадочные направлений /sektsii/risovanie и /sektsii/muzyka"),
       ("Меню разделов", "Спортивные, Художественные и Языковые секции — хабы каталога"),
       ("«Читайте также» внизу", "две соседние статьи: про спортивные и языковые секции")]
S.append(slide(top("Блог и посадочные") + f'<h1>Ссылка из статьи ведёт на посадочную с {fmt(D["besplatno"]["entries"])} входами из поиска в месяц</h1>'
  '<div class="body"><div class="row fill" style="gap:28px;align-items:center">'
  f'<div style="flex:1.1">{browser("мапка.рф/blog/besplatnye-sekcii-dlya-detey-rostov", "mapka_article_link.webp")}</div>'
  f'<div style="flex:1;display:flex;flex-direction:column;gap:18px"><div style="font:500 19px/1.3 var(--ft);color:var(--slate)">Куда ведут 26 внутренних ссылок статьи</div>{plist(LNK, "link")}'
  '<p style="font:400 19px/1.45 var(--ft);color:var(--charcoal)">Статья ставит одну-три ссылки на посадочные, текст ссылки — запрос этой посадочной. Посадочная отвечает ссылками на одну-две статьи. Так информационный трафик доходит до страницы, которая продаёт.</p></div>'
  '</div><p class="foot">Ссылка подсвечена нами. Снимок и список ссылок — с живой страницы, 24 сентября 2026.</p></div>', "Блог и посадочные"))

# статья под ИИ
GEO = [("Ответ в первых ста словах", "так у 9 из 10 самых цитируемых страниц"),
       ("Шаги, списки, таблицы", "подборки «топ-N» — 63% всех цитат"),
       ("Свои цифры, от трёх", "вчетверо больше цитат"),
       ("Автор на странице", "около +40% к цитированию"),
       ("Обновление раз в сезон", "95% цитат — страницы моложе 10 месяцев"),
       ("FAQ дословно как запросы", "ассистент ищет подзапросами"),
       ("Весь текст в HTML", "ChatGPT не выполняет скрипты")]
S.append(slide(top("Статья под ИИ-ответ") + '<h1>Под ИИ-ответ статью пишут строже: ответ в начале, свои цифры, автор и дата</h1>'
  '<div class="body"><div class="row fill" style="gap:28px;align-items:center">'
  f'<div style="flex:.85">{browser("techkio.ru/guide/create-press · частые вопросы", "techkio_faq.webp")}</div>'
  '<div style="flex:1.1"><div class="tbl dense" style="grid-template-columns:minmax(0,1fr) minmax(0,1fr)">'
  '<div class="h">Что делать</div><div class="h">Почему</div>'
  + "".join(f'<div class="k" style="font-size:19px">{a}</div><div style="font-size:19px">{b}</div>' for a, b in GEO)
  + '</div></div></div><p class="foot">Сводка исследований цитируемости в ответах ИИ-ассистентов, август 2026. Подробно — на встрече 5.</p></div>', "Статья под ИИ-ответ"))

S.append(divider("06", "Практикум", "Собираем ядро и дерево для ниши, которую выбрали в начале.",
                 ["Шаг 1: тридцать запросов", "Шаг 2: таблица ядра", "Шаг 3: дерево страниц", "Разбор на экране"]))

# 38 шаг 1
S.append(slide(top("Практикум, шаг 1") + '<h1>Шаг 1. Тридцать запросов из Вордстата за десять минут</h1>'
  '<div class="body"><div class="pipe" style="margin:auto 0 22px">'
  + step("1", "Маркеры", "Пять-семь слов о нише: как её называют клиенты. Можно спросить ИИ.", "teal", "pen", 0) + arrow()
  + step("2", "Топ запросов", "Каждый маркер в Вордстат, регион — Ростов. Выписываем всё про нашу услугу.", "teal", "search", 1) + arrow()
  + step("3", "Точная частота", "Кавычки и восклицательные знаки, мусор — в поле минус-слов.", "teal", "gauge", 2)
...
CHECK = [("Одному кластеру — одна страница", "Нет двух страниц под одно намерение."),
         ("Вопросы — в статьи", "Каждая статья ссылается на одну-три посадочные."),
         ("Важное в трёх кликах", "От главной до любой услуги."),
         ("Ни одной сироты", "У каждой страницы минимум два входа: раздел и соседняя страница или статья."),
         ("Проверка", "Когда сайт готов — SiteAnalyzer по главной, разделам и одной посадочной.")]
...
DEAD = [("Ядро на тысячи ключей без кластеров", "Таблица есть, а какую страницу под что делать, непонятно."),
        ...
        ("Страница без входящих ссылок", "Сирота: робот находит её поздно, вес до неё не доходит."),
        ("Блог ради объёма текста", "Статья без ссылок на посадочные не приводит покупателей."),
        ("Красота, которой нет в HTML", "Текст в картинках и вкладках на скриптах поисковик не читает.")]
ALIVE = [("Кластеризация по выдаче", "Одна страница или две — решает первая десятка."),
         ("Одна страница — одно намерение", "Коммерческое на посадочные, информационное в статьи."),
         ("Посадочная для человека и для робота", "Каждый блок продаёт и одновременно отдаёт текст и ссылки."),
         ("Статья ведёт на 1–3 посадочные", "Текст ссылки — запрос посадочной, обратно ссылка на статью."),
...
SRCS = [("Яндекс Вебмастер: запросы, показы, клики", "webmaster.yandex.ru", "мапка.рф и techkio.ru, 13.08–12.09.2026"),
        ...
        ("Расширение SiteAnalyzer SEO Tools", "site-analyzer.pro", "магазин Chrome, 24.09.2026"),
        ("Что влияет на цитирование в ИИ-ответах", "сводка исследований", "август 2026"),
...
# ═══ сборка: Вордстат → кластеры → страницы → блог → ссылки и проверка → практикум ═══
# что ещё бывает кроме Вордстата
MORE = [(["wordstat"], "Частота, топ запросов, регионы, сезонность", "бесплатно", "Хватит для учебного проекта"),
        (["webmaster"], "По каким запросам сайт уже показывают", "бесплатно", "Когда сайт запущен"),
        (["bukvarix"], "Готовая база больше 2 млрд фраз, выгрузка списком", "бесплатный режим", "Если не хочется копировать руками"),
        (["keysso"], "По каким запросам видны конкуренты", "платно", "Проект с бюджетом"),
        (["topvisor", "rush"], "Кластеризация по выдаче сама, без ручных проверок", "платно", "Ядро на тысячи запросов"),
        (["kc"], "Сбор больших ядер программой на компьютере", "платная лицензия", "Агентства и большие сайты")]
def more_row(slugs, what, price, when):
    hl = ' style="background:var(--teal-l)"' if slugs == ["wordstat"] else ""
    pc = "var(--moss)" if price.startswith("бесплат") else "var(--orange-d)"
    names = "<br>".join(f'<span style="color:{WORD[x][1]}">{WORD[x][0]}</span>' for x in slugs)
    return (f'<div class="k"{hl}>{names}</div>'
            f'<div{hl}>{what}</div><div{hl}><span style="font:500 19px/1.3 var(--ft);color:{pc}">{price}</span></div><div class="k"{hl}>{when}</div>')
S.append(slide(top("Что ещё бывает") + '<h1>Для учебного проекта хватит Вордстата. Платные нужны под тысячи запросов</h1>'
  '<div class="body"><div class="tbl dense" style="grid-template-columns:260px minmax(0,1.25fr) 210px minmax(0,1fr);margin:auto 0">'
  '<div class="h">Сервис</div><div class="h">Что даёт</div><div class="h">Цена</div><div class="h">Когда нужен</div>'
  + "".join(more_row(*r) for r in MORE)
  + '</div><p class="foot">Состав рынка и объёмы баз: key-core.ru и kokoc.com, обзоры сервисов 2026; toolfox.ru — Букварикс. На практикуме работаем только в Вордстате.</p></div>', "Что ещё бывает"))

S.append(divider("01", "Вордстат", "Заходим, вбиваем маркер, смотрим, что и как часто ищут в нашем городе.",
                 ["Шесть слов для начала", "Намерение и город в запросе", "Вордстат в 2026 году", "Операторы и точная частота", "Что ещё бывает, кроме Вордстата"]))
S.append(divider("02", "Кластеры", "Из собранного выкидываем мусор и раскладываем остальное по страницам.",
                 ["Чистка и минус-слова", "Кластер — это страница", "Одна страница или две", "Каннибализация"]))
S.append(divider("03", "Страницы", "Под каждый кластер своя страница: дерево сайта и посадочные.",
                 ["Дерево кластеров", "Типы страниц", "Посадочная: первый экран", "Посадочная: блок под картой", "Красиво и SEO-корректно"]))
S.append(divider("04", "Блог", "Зачем он сайту, который продаёт, какие бывают статьи и как их писать под поиск и под ИИ.",
                 ["Зачем блог коммерческому сайту", "Шесть типов статей", "Как пишется статья", "Разбор настоящего гайда", "Статья под ИИ-ответ"]))
S.append(divider("05", "Ссылки и проверка", "Связываем страницы между собой и проверяем, что ни одна не потерялась.",
                 ["Перелинковка по дереву", "Статья ведёт на посадочные", "Страницы-сироты", "Проверка расширением SiteAnalyzer"]))
S.append(divider("06", "Практикум", "Собираем ядро и дерево для ниши, которую выбрали в начале.",
                 ["Тридцать запросов из Вордстата", "Таблица ядра", "Дерево и проверка по чек-листу"]))

ORDER = ["Обложка", "План", "Длинный хвост", "Выбор ниши",
         "Раздел 01: Вордстат", "Словарь", "Намерение и город", "Вордстат 2026", "Операторы", "Три частоты", "Что ещё бывает",
         "Раздел 02: Кластеры", "Чистка", "Кластер = страница", "Кластеризация по выдаче", "Каннибализация",
         "Раздел 03: Страницы", "Дерево кластеров", "Типы страниц", "Посадочная: первый экран", "Посадочная: под картой", "Красиво и SEO-корректно",
         "Раздел 04: Блог", "Зачем блог", "Типы статей", "Как пишется статья", "Разбор гайда", "Статья под ИИ-ответ",
         "Раздел 05: Ссылки и проверка", "Перелинковка", "Блог и посадочные", "Страницы-сироты", "Расширение SiteAnalyzer",
         "Раздел 06: Практикум", "Шаг 1", "Шаг 2", "Шаг 3", "Домашка", "Анонс встречи 3", "Источники", "Вопросы"]
import re
BY = {html.unescape(re.search(r'data-t="([^"]*)"', x).group(1)): x for x in S}
miss = [t for t in ORDER if t not in BY]
assert not miss, miss
S = [BY[t] for t in ORDER]
